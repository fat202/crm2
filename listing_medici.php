...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Appuntamenti utente</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
			<form method="POST" action="">
      <div class="modal-body">
	   Selezionare l'informatore:
			<select id="basic" name="ute_id" class="form-control selectpicker form-select " data-live-search="true" placeholder="Inserisci il Campagna">
                          <option value=""></option>
                          <?php
                          $query =
                          ...
                          ?>
												...
												<br>
												Data e ora:
												<input type="datetime-local" name="com_app_chiamata_data_inizio" class="form-control" placeholder="Inserisci il Data Inizio della Campagna" />
												<br>
												<br>
												Selezionare il n° call per l'appuntamento:
												<select id="basic" name="ele_call_numero" class="form-control selectpicker form-select " data-live-search="true" placeholder="Inserisci il Campagna">
                          <option value=""></option>
                          <?php
                          $query = "SELECT * from crm_elenco_call ";
                          $res_crm = mysqli_query($conn, $query);
                          while ($crm = mysqli_fetch_array($res_crm)) { ?>
                            <option value="<?= $crm['ele_call_numero'] ?>" >
                              <?= $crm['ele_call_descr'] ?>
                            </option>
                          <?php }
                          ?>
												...
												<br>
												Stato appuntamento:
												<select id="basic" name="com_ele_sta_app_descr" class="form-control selectpicker form-select " data-live-search="true" placeholder="Inserisci il Campagna">
  ...
  <input class="form-check-input" name="com_app_video" type="checkbox" value="1" id="flexCheckDefault">

	<input type="hidden" name="com_app_video" value="0">
     ...
     if (mysqli_num_rows($query_run) > 0) {
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
				<button type="submit" name="submit_appuntamenti" class="btn tasto_carica_altri"><b>Crea Appuntamenti</b></button>
  ...
  <thead>
    <tr class="table-active">
      ...
      <th scope="col">INFORMATORE</th>
			...
			<th class="table-danger" scope="col">Edit/Delete</th>
    </tr>
  </thead>
  <tbody>
	<?php
 $query = "SELECT * FROM crm_comunicazione_appuntamenti where com_deleted = 0";
 $query_run = mysqli_query($con, $query);
 if (mysqli_num_rows($query_run) > 0) {
     foreach ($query_run as $crm) { ?>
   <tr>
      <th scope="row"><?= $crm['com_app_data'] ?></th>
      <td><?= $crm['com_ele_sta_app_descr'] ?> </td>
      <td>	
			<?= $crm['ute_informatori'] ?> 			</td>
      <td><?= $crm['ele_call_numero'] ?></td>
			<td></td>
			<td></td>
			<td> 
			<a type="button" href="appuntamenti_edit.php?com_app_id=<?= $crm['com_app_id']; ?>"><img src="images/pencil-square.svg" alt=""></a>

                    / 
                        <a type="button" data-bs-toggle="modal" data-bs-target="#exampleModal<?= $crm['com_app_id']; ?>">Delete<img src="images/x (1).svg" alt=""></a>
                     </td>
    </tr>

              <!-- Modal -->
              <div class="modal fade" id="exampleModal<?= $crm['com_app_id']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">

                  <div class="modal-content">

                    <div class="modal-header">
                      <h5 class="modal-title" id="exampleModalLabel"><?= $crm['com_app_id']; ?></h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      Do you want to delete <?= $crm['ute_informatori']; ?>?
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Close</button>
                      <form action="inc/create_utenti_anagrafica.inc.php" method="POST" class="d-inline">


                        <input type="hidden" name="id" value="<?= $crm['com_app_id']; ?>">
							...
							<div class="card-body">
								<div class="row" style="margin-bottom: 1rem;">
									...
									<div class="col-12 col-lg-5 mt-1 d-flex">
										...
										<div style="text-align:start; margin-left: 1rem;">
											<b class="font_size_20">GESTIONE RILEVAZIONE INFORMAZIONI</b><br>
										

											<?php if (isset($_GET['com_utn_id'])) {
     $com_utn_id = mysqli_real_escape_string($conn, $_GET['com_utn_id']);
     $query = "SELECT * FROM crm_survey WHERE com_utn_id='$com_utn_id'";
          $query_run = mysqli_query($conn, $query);
          if (mysqli_num_rows($query_run) > 0) {
            foreach ($query_run as $crm) {
          ...
          ?>
                        ...
                        <ul>
												...
												<li ><a  target="_blank" style="text-decoration: none; color: grey;" href="survey_risposte_utenti.php?sur_id=<?= $crm[ 'sur_id'] ?>&com_utn_id=<?= $crm[ 'com_utn_id'] ?>" class="survey"><?= $crm['sur_descr'] ?></a></li>
